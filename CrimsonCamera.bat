...
:: ============================================================
:: GAME DIRECTORY (fully automatic detection)
:: ============================================================
set "GAMEDIR="
set "CONFIGFILE=%SD%gamepath.txt"

echo.
echo   Detecting Crimson Desert installation...

:: 1. Check saved path from previous run
if exist "!CONFIGFILE!" (
    set /p "GAMEDIR=" <"!CONFIGFILE!"
    if exist "!GAMEDIR!\0010\0.paz" (
        echo   Found (cached^): !GAMEDIR!
        goto GAMEDIR_OK
    )
    echo   Saved path no longer valid, re-detecting...
    set "GAMEDIR="
)

:: 2. Check if we're inside the game folder (backward compatible)
for %%I in ("%SD%..") do set "TRYDIR=%%~fI"
if exist "!TRYDIR!\0010\0.paz" (
    set "GAMEDIR=!TRYDIR!"
    goto GAMEDIR_SAVE
)

:: 3. Smart detection: Steam registry, Epic manifests, Xbox/Game Pass
for /f "delims=" %%G in ('powershell -NoProfile -ExecutionPolicy Bypass -File "%SD%lib\find_game.ps1" 2^>nul') do (
    set "GAMEDIR=%%G"
)
if defined GAMEDIR if exist "!GAMEDIR!\0010\0.paz" goto GAMEDIR_SAVE

:: 4. Brute-force scan common paths on all fixed drives
for %%D in (C D E F G H I) do (
    if "!GAMEDIR!"=="" (
        for %%P in (
            ...
            "%%D:\Program Files\Epic Games\CrimsonDesert"
            ...
            if "!GAMEDIR!"=="" if exist "%%~P\0010\0.paz" set "GAMEDIR=%%~P"
        )
    )
)
if defined GAMEDIR goto GAMEDIR_SAVE

:: All detection methods failed
echo.
echo  ============================================================
echo   ERROR: Crimson Desert installation not found.
echo  ============================================================
echo.
echo   Searched: Steam, Epic Games Store, Xbox/Game Pass,
echo   and common install paths on drives C-I.
echo.
echo   If your game is installed in an unusual location, create
echo   a file called "gamepath.txt" next to CrimsonCamera.bat with the
echo   full path to the game folder on a single line.
echo.
pause
exit /b
...
echo.
echo   To change settings: run CrimsonCamera.bat again
